      ...
      <div className="max-w-4xl mx-auto px-5 py-16">
        ...
        <div className="bg-white rounded-2xl border border-plum-100 p-8 mb-8">
          ...
          <p className="text-sm text-plum-500 mb-6">
            Use the Excel template exported from Colab. Existing products with the same URL will be updated.
          </p>

          {/* Expected format */}
          <div className="bg-plum-50 rounded-xl p-4 mb-6 text-xs text-plum-700 font-mono overflow-x-auto">
                ...
                <thead>
                  <tr className="border-b border-plum-100">
                    {Object.keys(preview[0]).map((k) => (
                      <th key={k} className="text-left text-plum-500 font-medium pb-2 pr-4 whitespace-nowrap">{k}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {preview.map((row, i) => (
                    <tr key={i} className="border-b border-plum-50">
                      {Object.values(row).map((v, j) => (
                        <td key={j} className="py-2 pr-4 text-plum-700 max-w-xs truncate">{String(v)}</td>
                      ))}
                    </tr>
                  ))}
                </tbody>
